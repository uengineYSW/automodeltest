<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>OperationRecordId</v-list-item-title>
                    </v-list-item-content>
                </template>
                ...
                <v-list-item v-for="(item, idx) in value" :key="idx">
                    <v-list-item-content>
                        ...
                        <v-list-item-subtitle>
                            OperationId :  {{item.operationId }} <br>
                        </v-list-item-subtitle>
                    </v-list-item-content>
        ...
        <OperationRecordId v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import OperationRecordId from '../OperationRecordId.vue';

    export default {
        name: 'OperationRecordIdManager',
        components: {
            OperationRecordId
        },
        props: {
