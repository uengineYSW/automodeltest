<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>VehicleId</v-list-item-title>
                    </v-list-item-content>
                </template>
                ...
                <v-list-item v-for="(item, idx) in value" :key="idx">
                    <v-list-item-content>
                        ...
                        <v-list-item-subtitle>
                            VehicleId :  {{item.vehicleId }} <br>
                            LicensePlate :  {{item.licensePlate }} <br>
                            OwnerName :  {{item.ownerName }} <br>
                        </v-list-item-subtitle>
                    </v-list-item-content>
        ...
        <VehicleId v-if="editMode && tick" offline :isNew="true" :inList="true"
