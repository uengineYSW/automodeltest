<template>
    <v-card outlined>
        <v-card-title>
            AssignmentRequest # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <RequestStatus offline label="Status" v-model="item.status" :editMode="false" :key="false" @change="change" />
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/assignmentRequests/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
