<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>OperationSummaryId</v-list-item-title>
                    </v-list-item-content>
                </template>
        ...
        <OperationSummaryId v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import OperationSummaryId from '../OperationSummaryId.vue';

    export default {
        name: 'OperationSummaryIdManager',
        components: {
            OperationSummaryId
        },
        props: {
